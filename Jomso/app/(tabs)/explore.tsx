...
export default function ExploreScreen() {
  ...
  function getDistanceFromUser(bin: Bin): string {
    ...
    const to = { latitude: bin.lat, longitude: bin.lng };
    const distance = haversine(from, to);
  ...
  function findClosestBin(): Bin | null {
    ...
    return bins.reduce((closest, bin) => {
      ...
      const closestDistance = closest
        ? haversine(from, { latitude: closest.lat, longitude: closest.lng })
        : Infinity;
      return distance < closestDistance ? bin : closest;
    }, null as Bin | null);
  }
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={StyleSheet.absoluteFillObject} pointerEvents='box-none'>
        <BottomSheet
          ...
          backgroundStyle={{ backgroundColor: colors.bg_tertiary }}>
          <View style={{ flex: 1 }}>
            {selectedBin ? (
              <BinSheetContent
                bin={selectedBin}
